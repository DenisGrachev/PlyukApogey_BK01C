...
namespace sprConv
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //Bitmap bitmap = new Bitmap("heroIdleSprite.png");
            Bitmap bitmap = new Bitmap(args[0]);
            int width = bitmap.Width;
            int height = bitmap.Height;

            int totalFrames = width / 12;
            ...
            for (int x = 0; x < totalFrames; x++)
            {
                ...
                for (int yy = 0; yy < height/2; yy++)
                {
                    for (int xx = 0; xx < 4; xx++)                    
                    {
                        ...
                        //any color
                        Color pixel0 = bitmap.GetPixel(x * 12 + xx * 3 + 0,yy * 2 + 0);
